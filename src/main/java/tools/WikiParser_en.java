...
public class WikiParser_en {
	
	private List<Category> categories;
	private List<WikiPage> pages;
	
	private static final Logger log = Logger.getLogger(WikiParser_en.class.getName());

	public WikiParser_en(){
	...
	public void calculateCategories(String mainCategory, String additionalPageNum) {
		...
		try {
			if (additionalPageNum.equals("")) 
				query = "https://en.wikipedia.org/w/api.php?format=json&action=query&"
					+ "list=categorymembers&cmtitle=Category:" + mainCategory
					+ "&cmtype=subcat&rawcontinue";
	...
	public String getExtractById(String id) {
		String query = "https://en.wikipedia.org/w/api.php?format=json&action=query&pageids=" 
			+ id + "&prop=extracts";
		...
		String extract = obj3.getString("extract");
	...
	public byte[] getPicture(String text) {
		...
		String query = "https://en.wikipedia.org/w/api.php?format=json&action=query&titles=" + text + "&prop=pageimages";
